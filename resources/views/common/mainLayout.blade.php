...
<head>
	...
	<meta name="author" content="Lluís Fontboté">
	@yield('description')
	@yield('keywords')
	...
	<nav class="bg-gray-900 p-4 mt-0 w-full">
        <div class="container mx-auto flex items-center">
		    <div class="flex text-white font-extrabold">
				<a class="flex text-white text-base no-underline hover:text-white hover:no-underline" href="{{ route('app.index') }}"> 
			...
			<div class="flex pl-4 text-sm">
				<ul class="list-reset flex justify-between flex-1 md:flex-none items-center">
				  <li class="mr-2">
					<a class="inline-block py-2 px-2 text-white no-underline" href="{{ route('app.index') }}">Inici</a>
				  </li>
				  @foreach ($categories as $categoria)
				  <li class="mr-2">
					<a class="inline-block text-gray-600 no-underline hover:text-gray-200 hover:text-underline py-2 px-2" href="{{ route('categoria.show', $categoria->id) }}">{{ $categoria->title }}</a>
				  </li>
				  @endforeach
				</ul>
			</div>
